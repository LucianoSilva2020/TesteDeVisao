function startContrast(test){
    if(!didTutorial){
        tutorial(test, 'acuity')
        return
    }
    if(rounds > roundsPlayed){
        starRoundsContrast(test)
        roundsPlayed++
    } else {
        if(testEye == 0){
            $("#staticEye").innerHTML = ""
            ...
            $("main").innerHTML =`
                <div class="containerResult">
                    <div class="titleResult">
                        <img src="./src/imagens/contrast-test.svg">
...
function testNexeEyeContrast(test){
    testEye = 1
    roundsPlayed = 0
    startContrast(test)
}
function starRoundsContrast(test){
    $("#staticEye").innerHTML = `
...
`
    ...
    $("#question").style.opacity += `${1.1 - (roundsPlayed / 10)}`
